...
//        cacheRepository.setExpire(key, code, CacheRepository.DEF_TIMEOUT_5M);
        ...
        String key = CacheKey.buildKey(data.getType().name(), data.getMobile());
        cache.set(CacheKey.REGISTER_USER, key, code);
        return success();
    }
...
//        String code = cacheRepository.get(key);
        String key = CacheKey.buildKey(data.getType().name(), data.getMobile());
        CacheObject cacheObject = cache.get(CacheKey.REGISTER_USER, key);
        if (cacheObject.getValue() == null) {
            return success(false);
        }
        return success(data.getCode().equals(cacheObject.getValue()));
    }
}
