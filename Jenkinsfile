...
//       ACTION：   在部署服务器执行(停止、启动、重启等)动作  （none表示不执行动作）
        ...
        // 以下变量基本不变
        SERVER_NAME = "lamp-boot"

        // 推送到服务器端的文件夹路径
        remoteDirectory = "./temp_jar/"

        BOOT_SERVER = "${SERVER_NAME}-server"
        JAR_NAME = "${BOOT_SERVER}-none"
        // 推送时需要忽略的项目前缀
        removePrefix = "${BOOT_SERVER}/target/"

        // 需要推送到服务器端的文件(jar)
        sourceFiles = "${BOOT_SERVER}/target/${JAR_NAME}.jar"
    }
            ...
            steps {
                script {
                    if ("${MAVEN_COMMAND}" != "none") {
                        sh "mvn clean ${MAVEN_COMMAND} -T8 -Dmaven.compile.fork=true -Dmaven.test.skip=true -P ${PROFILES}"
                    } else {
